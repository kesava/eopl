...
(define-datatype expression expression?
  ...
  (traceproc-exp (ids (list-of symbol?)) (body expression?))
...
(define-datatype procval procval?
  (closure (ids (list-of symbol?))
           (body expression?)
           (env environment?))
  (trace-closure (ids (list-of symbol?))
           (body expression?)
           (env environment?)))
...
(define eval-expression
  (lambda (exp env)
    (cases expression exp
      ...
      (traceproc-exp (ids body) (trace-closure ids body env))
      (app-exp (rator rands)
...
(define apply-procval
  (lambda (proc args)
    (cases procval proc
      ...
      (trace-closure (ids body env)
               (if (= (length ids) (length args))
                   (begin
                     (display "Starting Proc\n")
                     (let ((result (eval-expression body (extend-env ids args env))))
                       (display "Ending Proc\n")
                       result))
                   (eopl:error "Wrong number of arguments"))))))
