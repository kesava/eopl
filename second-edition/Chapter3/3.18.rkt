...
(define upack
  (lambda (ids exp)
        (if (= (length exp) (length ids))
            exp
            (eopl:error "Provide equal number of ids and exps"))))
...
(define-datatype expression expression?
  ...
  (unpack-exp (ids (list-of symbol?)) (exps (list-of expression?)) (body expression?))
...
(define grammar-3-1
  '((program (expression) a-program)
    ...
    (expression ("unpack" (separated-list identifier ",") "=" "{" (separated-list expression ",") "}" "in" expression) unpack-exp)
...
; -->unpack a,b = {2,3} in +(a,b)
; 5
; -->unpack a,b = {2,+(45,40)} in +(a,b)
; \
